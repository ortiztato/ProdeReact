...
const Partido = (props) => {
  ...
  const addOpcion = (
    ...
    let seleccion = [];
    if (opcion1) {
      seleccion.push(partido1);
    }
    if (opcion2) {
      seleccion.push(partido2);
    }
    if (opcion3) {
      seleccion.push(partido3);
    }
  ...
  return (
    <div>
      {modoUser ? (
        ...
        <div className={classes.divpartido}>
          <div className={classes.nombreequipo}>{props.equipo1}</div>
          <input name="uno" type="checkbox" onChange={handleInputChange1} />
          <label for="unocheck"></label>
          <input name="dos" type="checkbox" onChange={handleInputChange2} />
          <input name="tres" type="checkbox" onChange={handleInputChange3} />
          <div className={classes.nombreequipo}>{props.equipo2}</div>
          <div>{okpartido ? "✔️​" : "❌"}</div>
        </div>
      )}
